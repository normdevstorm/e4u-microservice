...
public class GatewayserviceApplication {
        ...
        public RouteLocator customRouteLocator(RouteLocatorBuilder builder, LoggingGatewayFilterFactory loggingFactory,
                ...
                return builder.routes()
                                .route("auth-route", r -> r.path("/auth/**")
                                                .filters(f -> f.stripPrefix(1)
                                                                .filter(loggingFactory.apply(
                                                                                new LoggingGatewayFilterFactory.Config()))
                                ...
                                .route("curriculum-route", r -> r.path("/curriculum/**")
                                                .filters(f -> f.stripPrefix(1)
                                                                .filter(loggingFactory.apply(
                                                                                new LoggingGatewayFilterFactory.Config()))
                                                // .filter(authenticationFilter.apply(
                                ...
                                .route("lesson-route", r -> r.path("/lesson/**")
                                                .filters(f -> f.stripPrefix(1)
                                                                .filter(loggingFactory.apply(
                                                                                new LoggingGatewayFilterFactory.Config()))
                                                // .filter(authenticationFilter.apply(
                                ...
                                .route("openapi", r -> r.path("/v3/api-docs/**")
                                                .filters(f -> f.rewritePath("/v3/api-docs/(?<service>.*)",
                                                                "/${service}/v3/api-docs"))
