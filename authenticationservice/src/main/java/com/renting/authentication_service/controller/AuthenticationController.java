...
public class AuthenticationController {
    ...
    public ResponseEntity<?> register(@Valid @RequestBody UserRequestDto registerUserDto) {
        try {
            ...
            GenericResponse<SignUpResponseDto> genericResponse = GenericResponse.<SignUpResponseDto>builder()
                    .data(signUpResponseDto).success(true).message("Sign up successfully").build();
    ...
    public ResponseEntity<RefreshTokenResponse> refreshToken(@RequestBody Map<String, String> refreshToken) {
        RefreshTokenResponse refreshTokenResponse = authenticationService
    ...
    public ResponseEntity<GenericResponse<Boolean>> validateToken(@RequestBody Map<String, String> token) {
        ...
        if (isValid) {
            return ResponseEntity
                    .ok(GenericResponse.<Boolean>builder().data(true).success(true).message("Token valid").build());
        }
        return ResponseEntity
                .ok(GenericResponse.<Boolean>builder().success(true).data(false).message("Token invalid").build());
    }
