...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<GenericException> customThrowException(UsernameNotFoundException e) {
        GenericException exceptionGenericResponse = GenericException.builder().timestamp(new Date())
                .status(HttpStatus.NOT_FOUND).details(e.getCause().toString()).message(e.getMessage()).build();
    ...
    public ResponseEntity<GenericException> badCredentialsExceptionHandler(BadCredentialsException e) {
        GenericException exceptionGenericResponse = GenericException.builder().timestamp(new Date())
                .status(HttpStatus.BAD_REQUEST).details("error").message(e.getMessage()).build();
    ...
    public ResponseEntity<GenericException> malformedJwtExceptionHandler(MalformedJwtException e) {
        GenericException exceptionGenericResponse = GenericException.builder().timestamp(new Date())
                .status(HttpStatus.UNAUTHORIZED).details("error").message(e.getMessage()).build();
    ...
    public ResponseEntity<GenericException> noSuchAlgorithmException(NoSuchAlgorithmException e) {
        GenericException exceptionGenericResponse = GenericException.builder().timestamp(new Date())
                .status(HttpStatus.INTERNAL_SERVER_ERROR).details("error").message(e.getMessage()).build();
    ...
    public ResponseEntity<GenericException> handleCustomJwtException(CustomJwtException e) {
        GenericException genericResponse = GenericException.builder().timestamp(new Date())
                .status(HttpStatus.UNAUTHORIZED).message(e.getMessage()).details(e.getMessage()).build();
        if (e.getCause() != null) {
    ...
    public ResponseEntity<GenericException> authorizationDeniedException(AuthorizationDeniedException e) {
        GenericException genericResponse = GenericException.builder().status(HttpStatus.FORBIDDEN).message("FORBIDDEN")
                .details(e.getMessage()).timestamp(Date.from(Instant.now())).build();
        return ResponseEntity.status(genericResponse.getStatus()).body(genericResponse);
    }
    ...
    public ResponseEntity<GenericException> entityNotFoundExceptionHandler(CustomEntityNotFoundException e) {
        GenericException genericResponse = GenericException.builder().status(HttpStatus.NOT_FOUND).message("NOT_FOUND")
                .details(e.getMessage()).timestamp(Date.from(Instant.now())).build();
        return ResponseEntity.status(genericResponse.getStatus()).body(genericResponse);
    }
    ...
    // GenericException genericResponse =
    // GenericException.builder().status(HttpStatus.BAD_REQUEST).message("BAD_REQUEST").details(e.getMessage()).timestamp(Date.from(Instant.now())).build();
    // return
    // ResponseEntity.status(genericResponse.getStatus()).body(genericResponse);
    // }
    ...
    protected ResponseEntity<Object> handleHttpMessageNotReadable(HttpMessageNotReadableException ex,
        ...
        GenericException genericResponse = GenericException.builder()
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
        ...
        GenericException genericResponse = GenericException.builder().status(HttpStatus.BAD_REQUEST)
                .message("BAD_REQUEST").details(invalidMessage.toString()).timestamp(Date.from(Instant.now())).build();
        return ResponseEntity.status(genericResponse.getStatus()).body(genericResponse);
    }
}
