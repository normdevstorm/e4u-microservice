...
public interface UserVocabProgressRepository extends JpaRepository<UserVocabProgress, UUID> {
       ...
       /**
        * Count words with intervalDays == 0 (brand new, never reviewed by SRS).
        */
       long countByUserIdAndIntervalDaysEquals(UUID userId, int intervalDays);

       /**
        * Count words currently "in learning" — SRS started but not mastered and not
        * overdue.
        * Criteria: intervalDays > 0, isMastered = false, nextReviewAt > now.
        */
       @Query("SELECT COUNT(uvp) FROM UserVocabProgress uvp " +
                     "WHERE uvp.userId = :userId " +
                     "AND uvp.isMastered = false " +
                     "AND uvp.intervalDays > 0 " +
                     "AND uvp.nextReviewAt > :now")
       long countLearningWords(@Param("userId") UUID userId, @Param("now") LocalDateTime now);

       /**
        * Count overdue words — not mastered and past their next review date.
        */
       @Query("SELECT COUNT(uvp) FROM UserVocabProgress uvp " +
                     "WHERE uvp.userId = :userId " +
                     "AND uvp.isMastered = false " +
                     "AND uvp.nextReviewAt <= :now")
       long countNeedsReviewWords(@Param("userId") UUID userId, @Param("now") LocalDateTime now);
}
