...
public interface WordContextTemplateRepository extends JpaRepository<WordContextTemplate, UUID> {
        ...
        /**
         * Count word contexts for a unit (shared contexts only)
         */
        @Query("SELECT COUNT(wct) FROM WordContextTemplate wct " +
                        "WHERE wct.unit.id = :unitId " +
                        "AND wct.createdForUserId IS NULL")
        long countByUnitIdShared(@Param("unitId") UUID unitId);

        /**
         * Count word contexts for a unit (shared + user-specific)
         */
        @Query("SELECT COUNT(wct) FROM WordContextTemplate wct " +
                        "WHERE wct.unit.id = :unitId " +
                        "AND (wct.createdForUserId IS NULL OR wct.createdForUserId = :userId)")
        long countByUnitIdForUser(
                        @Param("unitId") UUID unitId,
                        @Param("userId") UUID userId);
}
